...
class TransformVisualizer:
    ...
    def update_transform(self, device_id, T_world):
        ...
        if self.return_image:
            ...
            img_data = cv2.resize(img_data, (1280, 640), interpolation=cv2.INTER_AREA)
...
class Transformer:
    ...
    def trans(self, position, rotation, name):
        ...
        if is_left or is_right:
            if is_left and self.left_initial is None:
                self.left_initial = result_mat.copy()
            elif is_right and self.right_initial is None:
                self.right_initial = result_mat.copy()
            
            if self.left_initial is not None and self.right_initial is not None and not self.initial_transform_set:
                # 计算中间位置
                mid_pos = 0.5 * (self.left_initial[:3, 3] + self.right_initial[:3, 3])
                
                # 计算中间旋转（使用Slerp）
                r_left = R.from_matrix(self.left_initial[:3, :3])
                r_right = R.from_matrix(self.right_initial[:3, :3])
