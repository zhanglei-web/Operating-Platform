...
class AudioVisualizer:
    ...
    def setup_audio(self):
        if self.args.samplerate is None:
            self.args.samplerate = sd.query_devices(self.args.device, 'input')['default_samplerate']
...
def main():
    ...
    parser.add_argument('channels', type=int, default=[1], nargs='*', help='input channels (default: 1)')
    parser.add_argument('-d', '--device', type=int_or_str, help='input device')
    parser.add_argument('-w', '--window', type=float, default=200, help='window size (ms)')
    parser.add_argument('-i', '--interval', type=float, default=30, help='update interval (ms)')
    ...
    parser.add_argument('-n', '--downsample', type=int, default=10, help='downsampling factor')
